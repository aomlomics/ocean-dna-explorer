...
export default async function TaxonomyPage({ params }: { params: Promise<{ taxonomy: Taxonomy["taxonomy"] }> }) {
	...
	// Build search URLs - using correct tables and filter structure
	const analysesFilter = JSON.stringify([["taxonomy", "taxonomy", "contains", taxonomy]]);
	const analysesEncodedFilter = analysesFilter.replace(/\[/g, "%5B").replace(/\]/g, "%5D").replace(/,/g, "%2C").replace(/"/g, "%22");
	const analysesSearchUrl = `/search?table=analysis&advanced=${analysesEncodedFilter}`;
	
	const projectsFilter = JSON.stringify([["taxonomy", "taxonomy", "contains", taxonomy]]);
	const projectsEncodedFilter = projectsFilter.replace(/\[/g, "%5B").replace(/\]/g, "%5D").replace(/,/g, "%2C").replace(/"/g, "%22");
	const projectSearchUrl = `/search?table=project&advanced=${projectsEncodedFilter}`;
	
	const samplesFilter = JSON.stringify([["taxonomy", "taxonomy", "contains", taxonomy]]);
	const samplesEncodedFilter = samplesFilter.replace(/\[/g, "%5B").replace(/\]/g, "%5D").replace(/,/g, "%2C").replace(/"/g, "%22");
	const samplesSearchUrl = `/search?table=sample&advanced=${samplesEncodedFilter}`;

	// Get taxonomic data for ranking cards
	const taxonomicData = formatTaxonomyDisplay(dbTaxonomy);
		...
		<div className="container mx-auto py-6 space-y-6 max-w-full pb-8">
			...
			<div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
				...
				<div className="lg:col-span-2 space-y-4">
					<Map query={mapQuery} where={{ taxonomy }} cluster className="h-105 w-full rounded-lg" />

				{/* Cards Below Map */}
				<div className="grid grid-cols-3 gap-3">
					{/* Analyses Card */}
					<Link href={analysesSearchUrl}>
				...
				<div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4 items-stretch">
					...
					<div className="bg-base-200 rounded-lg p-6 flex flex-col">
						...
						<p className="text-xs text-base-content/60 text-left mb-4">
							Click each rank to explore related taxa!
						</p>
						<div className="space-y-4 flex-1">
							{taxonomicData.map((item) => {
								const isSpecies = item.rankKey === "species";
								const rankSearchUrl = `/explore/taxonomy?${item.rankKey}=${encodeURIComponent(item.name.replace(" ", "_"))}`;

								if (isSpecies) {
									return (
										<div key={item.rank} className="text-left rounded px-3 py-2 mb-3">
											<div className="text-[10px] font-medium text-base-content/50 uppercase tracking-widest">{item.rank}</div>
								...
								return (
									<Link key={item.rank} href={rankSearchUrl}>
										<div className="text-left cursor-pointer bg-base-300/30 hover:bg-base-300/60 transition-colors rounded px-3 py-2 mb-3">
											<div className="text-[10px] font-medium text-base-content/50 uppercase tracking-widest">{item.rank}</div>
											<div className="text-sm font-semibold text-primary hover:text-primary-focus transition-colors">{item.name}</div>
										</div>
									</Link>
								);
							})}
