...
export default async function EditHistory({ relationField, entry }: { relationField: string; entry: string }) {
	const editHistory = await prisma.edit.findMany({
		where: {
			[relationField]: entry
		},
		orderBy: {
								...
								{edit.changes.map((c, i) => (
									<div key={c.field + i} className="pl-8 flex flex-col items-start gap-1">
										...
										<div className="flex">
											...
											<p className="bg-base-200 px-2 py-1 rounded-md">{c.newValue}</p>
										</div>
									</div>
								))}
							</div>
