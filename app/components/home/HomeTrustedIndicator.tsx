...
export function TrustedShieldIcon({ trusted, className }: { trusted: boolean; className?: string }) {
	if (trusted) {
		return (
			<svg className={className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" aria-hidden="true">
				...
				<path d="M11.797 4.709l-.44-.378-.406.035-4.36 5.148-1.485-2.12-.4-.068-.463.331-.069.4 1.909 2.726.217.12.457.028.234-.102 4.835-5.715-.029-.405z" />
			</svg>
		);
	}
...
function modeLabel(trusted: boolean) {
...
export function ShowcaseTrustedLabel({ trusted }: { trusted: boolean }) {
	return (
		<p className="mt-1 flex items-center gap-2 text-base font-medium tracking-tight text-base-content/80 sm:text-lg">
			<TrustedShieldIcon trusted={trusted} className="h-6 w-6 shrink-0 fill-current text-primary" />
			Showing {modeLabel(trusted)}
		</p>
	);
}
