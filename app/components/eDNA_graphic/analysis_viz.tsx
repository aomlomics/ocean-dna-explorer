...
const AnalysisViz: React.FC<AnalysisVizProps> = ({ onBack, isActive }) => {
		...
		const timer = setInterval(() => {
			setAnimationStep((prevStep) => {
				...
				if (nextStep >= ALL_OUTLINES.length - 1) {
					clearInterval(timer);
					setTimeout(() => setShowFinalContent(true), FINAL_FADE_IN_DELAY);
				}
	...
	return (
		<div className="relative w-full h-full flex items-center justify-center p-8">
			...
			<div className="relative w-340 h-[50em]">
				...
				>
					...
					>
						<div className="flex flex-col h-full w-full origin-top scale-[0.96] px-6 pt-4 pb-8">
							<h3 className="text-3xl font-normal text-primary whitespace-nowrap">Common Bottlenose Dolphin</h3>
							<div className="grow flex items-center gap-8 mt-4">
								...
								<div className="w-1/3 h-full bg-base-200/40 rounded-lg flex flex-col items-center justify-center p-4 gap-4">
									<div
										className="w-full grow bg-primary"
										style={{
									...
									/>
									<p className="text-2xl font-bold text-success shrink-0">ASSIGNED</p>
								</div>
								...
								<div className="w-2/3">
									<div className="space-y-2 text-lg">
										{TAXONOMY_DATA.map((item) => (
											<div key={item.rank} className="grid grid-cols-2 items-baseline">
												<span className="font-semibold text-base-content/70">{item.rank}</span>
												<span className="font-mono text-base-content">{item.value}</span>
											</div>
										))}
									</div>
								</div>
					...
					<div
						className={`absolute inset-0 flex flex-col items-center justify-center transition-opacity duration-300 ${
							showFinalContent ? "opacity-0 pointer-events-none" : "opacity-100"
						}`}
					>
						<div className="text-3xl font-semibold text-base-content/70 mb-6">Assigning taxonomy...</div>
						<div
							key={currentOutline}
...
export const AssigningTaxonomyLoop: React.FC = () => {
	...
	return (
		<div className="relative w-full h-full flex items-center justify-center">
			<div className="relative w-md h-72 text-primary">
				{/* Laptop frame */}
