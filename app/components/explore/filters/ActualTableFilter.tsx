...
function ActiveFilterSummaries({ summaries, className = "max-w-lg" }: { summaries: string[]; className?: string }) {
	...
	return (
		<div ref={containerRef} className={`flex items-center gap-1 text-sm flex-wrap overflow-hidden ${className}`}>
...
export default function ActualTableFilter({
	...
	const showBody = embedded || isOpen;
				...
				>
					...
					<div className="flex items-center gap-4">
						...
						<svg
							...
							strokeLinecap="round"
							strokeLinejoin="round"
							className={`w-6 h-6 transition-transform text-base-content ${showBody ? "rotate-180" : ""}`}
						>
							<path d="M6 9l6 6 6-6" />
						</svg>
					</div>
