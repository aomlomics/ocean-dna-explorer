						...
						<>
							...
							<Callout title="The API and the website have opposite defaults">
								<p>
									...
									that have not been reviewed yet. Add <code className="px-1 py-0.5 bg-base-300 rounded">trusted=true</code> if you want your results to match what
									the website shows.
								</p>
							</Callout>
							...
							<Callout title="In a browser, the trusted toggle wins">
								<p>
									...
									you paste into the address bar. If your toggle is off, <code className="px-1 py-0.5 bg-base-300 rounded">trusted=true</code> still returns
									untrusted data.
								</p>
								<p>
				...
				},
				{
					...
					content: (
						<>
							...
							<div className="mb-4">
								Example URL:{" "}
								<InlineCode code={`${process.env.NEXT_PUBLIC_URL}/api/project?fields=id,project_name,institution&limit=5`} />
							</div>
					...
					content: (
						<>
							...
							<p className="mb-4">
								...
								includes every table along the way. By default each step returns only its <code className="px-1 py-0.5 bg-base-300 rounded">id</code>, which keeps
								the response small while still showing you the shape of the connection.
							</p>

							<p className="mb-4">
								<code className="px-1 py-0.5 bg-base-300 rounded">limit</code> caps records from the table in the URL. It does not cap the related rows nested
								inside them. One project can still come back with every sample it has.
							</p>

							<p className="mb-4">
								Analysis and Sample are joined through Library. <code className="px-1 py-0.5 bg-base-300 rounded">/api/analysis?relations=sample</code> returns each
							...
							<p className="mb-4">
								The loaded example is a library and its sample. A library has one sample, so <code className="px-1 py-0.5 bg-base-300 rounded">limit=1</code> is
								one library and one sample id.
							</p>

							<div className="mb-4">
								Example URL:{" "}
								<InlineCode code={`${process.env.NEXT_PUBLIC_URL}/api/library?fields=id,lib_id&relations=sample&limit=1`} />
							</div>
					...
					content: (
						<>
							<OptionSummary
								...
								rules={
									<>
										Both require <code className="px-1 py-0.5 bg-base-300 rounded">relations</code>, and they cannot be used together. Every table you name must
										also appear in <code className="px-1 py-0.5 bg-base-300 rounded">relations</code>.
									</>
								}
							/>

							<p className="mb-4">
								<code className="px-1 py-0.5 bg-base-300 rounded">relationsFields</code> picks specific fields from one related table. The first value is the table
								name and the rest are its fields. Repeat the whole option to cover more than one table.
							</p>
							...
							<p className="mt-6 mb-4">
								<code className="px-1 py-0.5 bg-base-300 rounded">relationsAllFields</code> is the shortcut for when you want everything. Set it to{" "}
								<code className="px-1 py-0.5 bg-base-300 rounded">true</code> for all fields on all requested relations, or give it a comma separated list of tables
								to limit it.
							</p>
							...
							<Callout title="Tables in the middle of a path stay as IDs">
								<p>
									...
									return just an <code className="px-1 py-0.5 bg-base-300 rounded">id</code>. <code className="px-1 py-0.5 bg-base-300 rounded">relationsAllFields=true</code> expands every table on the path,
							...
							<OptionSummary
								...
								rules={
									<>
										...
										<code className="px-1 py-0.5 bg-base-300 rounded">/api/❮table❯/relations</code> to see which tables qualify.
									</>
								}
							/>

							<p className="mb-4">
								Adds a <code className="px-1 py-0.5 bg-base-300 rounded">_count</code> object to each record with the number of related records, without returning
								the records themselves. This is much cheaper than pulling the relations and counting them yourself.
							</p>
					...
					content: (
						<>
							...
							<p className="mb-4">
								...
								many related records each row has, so <code className="px-1 py-0.5 bg-base-300 rounded">orderBy=Samples,desc</code> puts the largest projects
								first.
							</p>

							<p className="mb-4">
								Relation names are the one place capitalization matters. Use the exact name returned by{" "}
								<code className="px-1 py-0.5 bg-base-300 rounded">/api/❮table❯/relations</code>, such as <code className="px-1 py-0.5 bg-base-300 rounded">Samples</code> rather than <code className="px-1 py-0.5 bg-base-300 rounded">samples</code>.
							</p>
				...
				},
				{
					...
					content: (
						<>
							...
							<p className="mb-4">
								The difference matters for coordinates. <code className="px-1 py-0.5 bg-base-300 rounded">distinct=decimalLatitude</code> keeps one sample per
					...
					content: (
						<>
							<OptionSummary
								...
								rules={
									<>
										Both must be positive integers. <code className="px-1 py-0.5 bg-base-300 rounded">page</code> requires <code className="px-1 py-0.5 bg-base-300 rounded">limit</code>.
									</>
								}
							/>

							<p className="mb-4">
								<code className="px-1 py-0.5 bg-base-300 rounded">limit</code> caps how many records come back. <code className="px-1 py-0.5 bg-base-300 rounded">page</code> then steps through them in blocks
								of that size, starting at <code className="px-1 py-0.5 bg-base-300 rounded">page=1</code>. With <code className="px-1 py-0.5 bg-base-300 rounded">limit=5</code>, page 2 returns records 6
								through 10.
							</p>
				...
				},
				{
					...
					content: (
						<>
							...
							<p className="mb-4">
								The count endpoint does not accept <code className="px-1 py-0.5 bg-base-300 rounded">limit</code>. Without the flag this returns an error, with it
								the count is returned as normal.
							</p>
							<ApiCodeBlock
								language="json"
