...
async function doEdit(
	...
	try {
		...
		if (parseResult && parseResult.analysis.assay_name !== dbAnalysis.assay_name) {
			fetch(
				`${process.env.NEXT_PUBLIC_SERVER_URL}/analysis/${encodeURIComponent(analysis_run_name)}/afterSubmission${encodeURIComponent(dbAnalysis.assay_name)}?delete=true&skipDiversities=true&skipAllBlast=True`,
				{
					method: "POST",
					headers: {
