...
type RelationMetadata = Readonly<{
	field: string;
	table: ModelName;
...
type TableMetadataValue = Readonly<{
	plural: string;
	description: string;
...
function uncapitalizeTable(table: ModelName) {
	return (table.charAt(0).toLowerCase() + table.slice(1)) as Uncapitalize<ModelName>;
}

function capitalizeTable(table: Uncapitalize<ModelName>) {
...
const TableNames = Object.keys(TableMetadata) as Readonly<Uncapitalize<ModelName>[]>;
const NonDataTableNames = [
	"taxonomySpotlight",
	"tag",
	"alphaDiversity",
	"alphaDiversityIndex",
...
const pathPriorities = {
	Feature: {
		Taxonomy: ["Assignment", "Sample"]
	},
	Taxonomy: {
		Feature: ["Assignment", "Sample"]
	},
	Occurrence: {
		Assay: ["Analysis", "Library"]
	},
	AlphaDiversityIndex: {
		Analysis: ["AlphaDiversity", "Library"],
		Tag: ["AlphaDiversity", "Library"]
	}
} as Record<ModelName, Record<ModelName, ModelName[]>>;
...
function getRelationPath(start: Uncapitalize<ModelName>, target: Uncapitalize<ModelName>) {
	...
	while (queue.length) {
		...
		if (
			...
			for (const rel of [...TableMetadata[uncapitalizeTable(curr)].relations!].sort((a, b) => {
				const prio = pathPriorities[curr]?.[capsTarget];
				if (prio) {
					const aIndex = prio.indexOf(a.table);
					const bIndex = prio.indexOf(b.table);

					// Relations explicitly mentioned in the priority list
					// come before relations that aren't.
				...
				if (a.type.startsWith("many")) value++;
				if (a.type.endsWith("many")) value++;
				if (b.type.startsWith("many")) value--;
				if (b.type.endsWith("many")) value--;
				...
				if (
					...
					(curr !== "Assay" || //base case
						...
						rel.table === "BlastQuery" || //BlastQuery to Assay
						path[0] === "BlastQuery" || //starting at BlastQuery to Assay
						path[0] === "BlastQueryResult") && //starting at BlastQueryResult to Assay
					//Analysis
					...
					//Feature
					(curr !== "Feature" || //base case
						path.length === 1) && //starting at Feature
					//Taxonomy
					(curr !== "Taxonomy" || //base case
						path.length === 1 || //starting at Taxonomy
						rel.table === "TaxonomySpotlight" || //Taxonomy to TaxonomySpotlight
						path[0] === "TaxonomySpotlight") && //starting at TaxonomySpotlight to Taxonomy
					//disallow Sample -> Taxonomy -> Analysis
					(rel.table !== "Analysis" || path[path.length - 2] !== "Sample" || path[path.length - 1] !== "Taxonomy")
				) {
...
function toCode(value: unknown, indent: string): string {
	const childIndent = `${indent}\t`;

	if (Array.isArray(value)) {
		return `[\n${value.map((v) => `${childIndent}${toCode(v, childIndent)}`).join(",\n")}\n${indent}]`;
	}

	if (value !== null && typeof value === "object") {
		return `{\n${Object.entries(value)
			.map(([key, value]) => `${childIndent}${key}: ${toCode(value, childIndent)}`)
			.join(",\n")}\n${indent}}`;
	}
...
function metadataToCode() {
	return TableNames.map((table) => {
		...
		const entries = Object.entries(TableMetadata[table])
			.filter(([key]) => key !== "relationsSchema")
			.map(([key, value]) => {
				if (key === "schema") {
					return `${childIndent}${key}: PrismaZodTypes.${capsTable}Schema`;
				}

				if (key === "enumSchema") {
					return `${childIndent}${key}: PrismaZodTypes.${capsTable}ScalarFieldEnumSchema`;
				}

				return `${childIndent}${key}: ${toCode(value, childIndent)}`;
			});
...
export const NonDataTableNames = [${NonDataTableNames.map((t) => `"${t}"`)}] as const;
...
const outputPath = resolve(process.cwd(), "types/tableMetadata.ts");
