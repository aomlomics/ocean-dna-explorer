...
**Status:** ![DONE](https://img.shields.io/badge/%E2%9C%93_DONE-16a34a?style=flat-square) ![TODO](https://img.shields.io/badge/%E2%9C%97_TODO-dc2626?style=flat-square) ![WIP](https://img.shields.io/badge/%E2%86%BB_WIP-2563eb?style=flat-square) ![FALSE POSITIVE](https://img.shields.io/badge/%E2%89%A0_FALSE_POSITIVE-6b7280?style=flat-square) ![NOT PLANNED](https://img.shields.io/badge/%E2%80%94_NOT_PLANNED-6b7280?style=flat-square)

FALSE POSITIVE means the finding is not valid. NOT PLANNED means the finding is real, but we are not going to address it.
...
#### ![NOT PLANNED](https://img.shields.io/badge/%E2%80%94_NOT_PLANNED-6b7280?style=flat-square) P0-3 · Security / Privacy — `userIds` and `editHistory` are returned by the public data API
**Location**
...
#### ![FALSE POSITIVE](https://img.shields.io/badge/%E2%89%A0_FALSE_POSITIVE-6b7280?style=flat-square) P1-7 · Security / Reliability — BLAST request parameters are interpolated unencoded and unvalidated
...
| P2-13 | ![NOT PLANNED](https://img.shields.io/badge/%E2%80%94_NOT_PLANNED-6b7280?style=flat-square) | Code quality | ~20 `@ts-ignore` sites, notably `api/[table]/route.ts:63`, `pagination/route.ts:293`, `:310`, `:360`, `swapToTable/route.ts:20`; `explore/taxonomy/[taxonomy]/page.tsx:148`, `:169` (`(dbTaxonomy as any)[rank]`) | Dynamic `prisma[model]` access defeats type checking on the core public API | A typed model map (`Record<TableName, Delegate>`) restores safety at the boundary. These are load-bearing suppressions on the most-used code path, not incidental ones |
| P2-14 | ![DONE](https://img.shields.io/badge/%E2%9C%93-16a34a?style=flat-square) | Code quality | `explore/project/page.tsx:14`, `explore/sample/page.tsx:18` (`if (!projects) return <>Loading...</>`); `Table.tsx:917`, `:939` (`"test" === null`) | Unreachable branches: `findMany` always returns an array, and the string comparison is constantly false. Dead code rather than user-visible defects | Remove. Worth noting because these read as real states during review and obscure genuine error handling |
...
| P2-25 | ![FALSE POSITIVE](https://img.shields.io/badge/%E2%89%A0_FALSE_POSITIVE-6b7280?style=flat-square) | Cleanup | `BlastSearch.tsx:30` (unused `prevQueries` state), `:20-22` (in-repo TODOs: clear button, query history, use `useRouter` instead of `window.location.href`), `:227`; numeric inputs render `NaN` when unset (`:333`, `:372`, `:393`) | `NaN` appearing in a form field is the most user-visible of these. The `window.location.href` navigation at `:227` forces a full page reload where `useRouter` would perform a client transition | Number inputs given `NaN` render empty, which is the intended empty state — not a visible "NaN" string. Remaining notes (unused `prevQueries`, in-repo TODOs, `window.location.href`) are optional cleanup, not a user-facing defect |
